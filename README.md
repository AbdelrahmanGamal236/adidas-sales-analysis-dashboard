# Adidas Sales Analytics Dashboard
Interactive Power BI dashboard analyzing $120M Adidas sales data
![Overview](images/overview adidas1.1.png)

**Developed an interactive Power BI dashboard analyzing Adidas sales data across regions, products, and retailers, featuring KPIs like $120M total sales, 33M units sold, and 3.9% profit margin.**

## 📊 Key Features
- **3 Interactive Pages**: Overview, Product & Retailer Performance, Sales Analysis
- **Bookmarks**: Toggle between Sales vs. Profitability views
- **Visuals**: Maps with color saturation (Profit Margin%), YoY trends, Top states/products
- **DAX Measures**: Total Sales, Profit Margin %, Sales YoY Growth %, Distinct Retailers

## 🛠️ Tech Stack
- **Power BI Desktop** | DAX | Bookmarks | Slicers

## 📈 Business Insights
- Identified top-performing regions (West/Northeast) and products (Men's Streetwear)
- Optimized sales channels (Online > Outlet)
- Profitability analysis for strategic decisions

## 🚀 How to Use
1. Download `Adidas-Sales-Analysis.pbix`
2. Open in **Power BI Desktop** (latest version)
3. Interact with slicers/bookmarks to explore
---
⭐ **Star this repo** | 👨‍💻 **Built by [Your Name]** | 📧 **[your.email@example.com]**

